...
AddEventHandler('codem-craft:sendItem',function(a,b,c,d,e,f)
        if Config.frameworkObject == 'esx' then 
          ...
          local itemtest    
            ...
            if  success then
                TriggerClientEvent("codem-cyberhud:Notify123", src, Config.Notifications["success"]["message"], Config.Notifications["success"]["type"],Config.Notifications["success"]["time"])  
                local data = ExecuteSql("INSERT INTO `codem_craft` (`identifier`,`weaponname`,`weapontime`,`weaponlabel`,`itemTime`,`images`) VALUES ('"..identifier.."','"..a.."','"..b.."','"..c.."','"..time.."','"..e.."')")   
             
                local item = ExecuteSql("SELECT * FROM users WHERE identifier = '"..identifier.."'")
                if item[1] then
                    ExecuteSql("UPDATE  `users` SET `craftxp` = '" .. item[1].craftxp + f .. "' WHERE `identifier` = '"..identifier.."'" ) 
            ...
            local success = true    
              ...
              if  devam then
                  ...
                  if item[1] then
                      ExecuteSql("UPDATE  `players` SET `craftxp` = '" .. item[1].craftxp + f .. "' WHERE `citizenid` = '"..identifier.."'" ) 
  
                  end
              else
                  TriggerClientEvent("codem-cyberhud:Notify", src, Config.Notifications["error"]["message"], Config.Notifications["error"]["type"],Config.Notifications["error"]["time"])  
              end
